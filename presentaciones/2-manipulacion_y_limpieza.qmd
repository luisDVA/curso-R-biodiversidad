...
::: incremental
-   Código anidado (los resultados de evalúan de adentro hacia afuera)

-   Objetos intermedios (creamos objetos con resultados intermedios y los pasamos a la siguiente función en la secuencia)
...
notas_promedio_rd <- round(notas_promedio, digits = 2)
...
::: {style="text-align: right; background-color:#4a4a62; color:#e6e6d5; box-shadow: 10px 10px grey"} 
Adolfo Álvarez (2021)\
...
-   Abarca tareas que se repiten mucho en cualquier projecto de datos: importar archivos, limpiar datos, transformar, visualizar, o programar nuevas funciones.
...
![](imgs/data-science-communicate.svg)\
...
## Funciones y paquetes para peparar, transforar, y estructuras datos
...
-   Cargar el objeto `pinguinos` que viene dentro del paquete `datos`

-   Crear un subconjunto de datos que solo incluya `especie`, `isla`, y `largo_aleta_mm`

-   ¿Cuántos individuos tienen aletas \> a 195 mm?

-   Crae una nueva columna con el largo de aleta en pulgadas
...
[`dplyr` [selection features and helpers](https://dplyr.tidyverse.org/reference/dplyr_tidy_select.html)]{style="font-size: 1.3em; text-align: center"}
...
</br> Ese [gato]{style="color: orange"} feo
:::
:::
...
Si la entrada es **"pato"** y la E.R. es [pato]{style="color: orange"} </br> Habrá coincidencia cuando los caracteres [p]{style="color: orange"}, [a]{style="color: orange"}, [t]{style="color: orange"} y [o]{style="color: orange"} aparezcan consecutivamente en el texto de entrada
...
Un pequeño subconjunto de caracteres nos ayudan a describir patrones más complejos porque tienen comportamientos especiales y no se buscan solo a sí mismos. Cuando aparecen en una E.R., se interpretan de una manera especial.

Incluyen: \[\[\]\^\$.\|?\*+()\]\[d\]\]{style="color: orange"}, que están apartados para fines especiales
...
[ing\$]{style="color: orange"} empata "Boi[ing]{style="background-color: yellow"}" pero no "ingeniero"
...
[\\w]{style="color: orange; font-weight: bold"}  letras, guión bajo, y números<br> 
[\\d]{style="color: orange; font-weight: bold"}  dígitos<br> [\\t]{style="color: orange; font-weight: bold"}    tabulación [\\n]{style="color: orange; font-weight: bold"}   saltos de línea\
[\\s]{style="color: orange; font-weight: bold"}  espacio\
[\\b]{style="color: orange; font-weight: bold"}  límite de palabra<br>
...
str_detect(string = c("catalogo", "aguacate", "pera"), pattern = "cat")
```
...
pinguinos %>% 
  select(especie, matches("larg")) %>% sample_n(3)
```

## Filtrando filas 

Datos de paises (`paises`) de 📦 `datos`
...
paises %>% select(pais,continente) %>% sample_n(4)
```
